...
int LocallyGreedy(std::vector<int> &x, const mGraph &g, double &tlen, double &rprate) {
    ...
    // Allocate memory on CPU for the arrays and variable to be used in the CUDA kernel
    int *Delta, *unsat, *fixThre, *sumnd;
    Delta = new int[g.MAX_INDEX_OF_NODE];
    unsat = new int[g.MAX_INDEX_OF_NODE];
    fixThre = new int[g.MAX_INDEX_OF_NODE];
    sumnd = new int;
    *sumnd = 0;

    // Allocate memory on GPU for the arrays and variable to be used in the CUDA kernel
    int *d_Delta, *d_unsat, *d_fixThre, *d_sumnd;
    cudaMalloc(&d_Delta, g.MAX_INDEX_OF_NODE * sizeof(int));
    cudaMalloc(&d_unsat, g.MAX_INDEX_OF_NODE * sizeof(int));
    cudaMalloc(&d_fixThre, g.MAX_INDEX_OF_NODE * sizeof(int));
    cudaMalloc(&d_sumnd, sizeof(int));
    
    // Copy data from host to device
    cudaMemcpy(d_Delta, Delta, g.MAX_INDEX_OF_NODE * sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(d_unsat, unsat, g.MAX_INDEX_OF_NODE * sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(d_fixThre, fixThre, g.MAX_INDEX_OF_NODE * sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(d_sumnd, sumnd, sizeof(int), cudaMemcpyHostToDevice);

    // Invoke the CUDA kernel
    init_arrays<<<(g.MAX_INDEX_OF_NODE + 255) / 256, 256>>>(d_Delta, d_unsat, d_fixThre, g.MAX_INDEX_OF_NODE, g.V, d_sumnd);
    
    // Ensure all GPU computations are finished before proceeding
    cudaDeviceSynchronize();

    // Copy data from device to host
    cudaMemcpy(Delta, d_Delta, g.MAX_INDEX_OF_NODE * sizeof(int), cudaMemcpyDeviceToHost);
    cudaMemcpy(unsat, d_unsat, g.MAX_INDEX_OF_NODE * sizeof(int), cudaMemcpyDeviceToHost);
    cudaMemcpy(fixThre, d_fixThre, g.MAX_INDEX_OF_NODE * sizeof(int), cudaMemcpyDeviceToHost);
    // Copy back the sumnd result from device to host
    cudaMemcpy(sumnd, d_sumnd, sizeof(int), cudaMemcpyDeviceToHost);
